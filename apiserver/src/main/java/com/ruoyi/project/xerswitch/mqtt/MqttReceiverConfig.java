package com.ruoyi.project.xerswitch.mqtt;
...
import com.ruoyi.project.xerswitch.service.impl.XerSwitchServiceImpl;
...
public class MqttReceiverConfig {
    ...
    @Autowired
    private XerSwitchServiceImpl xerSwitchService;
    ...
    public MessageHandler handler() {
        return new MessageHandler() {
            ...
            public void handleMessage(Message<?> message) throws MessagingException {
                ...
                XerSwitch xerSwitch =xerSwitchService.selectXerSwitchById(Long.parseLong(deviceId));
                //添加和更新数据
                if (topic.equals("monitor")) {
                    //获取监测数据
                    XerSwLog log = JSON.parseObject(msg, XerSwLog.class);
                    xerSwitchLogService.saveObj(log);
                    ...
                    if (!switchA.equals("null") && switchA.length() != 0) {
                        xerSwitch.setSwitchA(switchA);
                        xerSwitch.setSwitchB(switchB);
                    }
                    xerSwitch.setState(state);
                    xerSwitchService.updateXerSwitch(xerSwitch);
                    //使用缓存则需要更新缓存+
